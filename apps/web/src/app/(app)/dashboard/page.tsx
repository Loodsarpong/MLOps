...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    api<SalesPoint[]>(`/reports/sales/daily?from=${from}&to=${to}`).then(setSales).catch(() => {});
    api<AgingRow[]>('/reports/ar/aging').then((r) => setAging(r.slice(0, 5))).catch(() => {});
    api<unknown[]>('/inventory/low-stock').then((r) => setLowStock(r.length)).catch(() => {});
  }, []);
  ...
  return (
    <div className="grid grid-cols-12 gap-4 p-6">
      <KpiCard className="col-span-3" label="Revenue (30d)" value={`GHS ${revenue30d.toFixed(2)}`} />
      <KpiCard className="col-span-3" label="Outstanding AR" value={`GHS ${arTotal.toFixed(2)}`} />
      <KpiCard className="col-span-3" label="Low-stock SKUs" value={String(lowStock)} />
